...
async function getAllTeams() {
  const teams = await Team.find().populate("members", "name _id profileImage").lean();
...
export function setupPresence(io: Server) {
  io.on("connection", async (socket: Socket) => {
    ...
    socket.on(
      "create-team",
      async ({ name, sport, location, maxPlayers, area }) => {
        console.log(
          "the user tryng to create team is ",
          userId,
          "locaiton is ",
          location,
        );
    ...
    socket.on("send-request",async (playerid)=>{
          console.log("is sendrequest controller")
            try {
              const sender = userId;
              const receiver = playerid;
              ...
              io.to(playerid).emit("receive-request",userId,friendship._id)
              
          
            } catch (err) {
              console.log("cant save req:",err);
            }
    })
